...
public class BlockLamp extends Block implements IHasModel {
    ...
    public void randomTick(World worldIn, BlockPos pos, IBlockState state, Random random) {
        if(!worldIn.isRemote) {
            if(ZoneEcho.dbUtils.getInteger("noLight") == 1) {
                worldIn.setBlockState(pos, BlockInit.TOP_LAMP_OFF.getDefaultState(), 2);
            } else {
                worldIn.setBlockState(pos, BlockInit.TOP_LAMP.getDefaultState(), 2);
            }
        }
    }
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        if(playerIn.inventory.getCurrentItem().equals(new ItemStack(ItemInit.WRENCH))) {
            worldIn.setBlockState(pos, BlockInit.TOP_LAMP_OFF.getDefaultState(), 2);
        }
        return false;
    }

...
public class BlockDoor extends Block implements IHasModel {
    ...
    {
        Objects.requireNonNull(worldIn.getTileEntity(pos)).getTileData().setBoolean("shouldupdate", true);
    ...
    {
        worldIn.setBlockState(pos, state.withProperty(FACING, placer.getHorizontalFacing()).withProperty(POWERED, Boolean.valueOf(false)), 2);
    }
    @Override
    ...
    public void neighborChanged(IBlockState state, World worldIn, BlockPos pos, Block blockIn, BlockPos fromPos)
    {
        if (!worldIn.isRemote)
        {
            if ((Boolean)state.getValue(POWERED).booleanValue() && !worldIn.isBlockPowered(pos))
            {
                ...
                worldIn.setBlockState(pos, BlockInit.DOOR.getDefaultState().withProperty(POWERED, Boolean.valueOf(true)).withProperty(FACING, state.getValue(FACING)), 2);
            }
        }
    }
    ...
    public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand)
    {
        if (!worldIn.isRemote)
        {
            if ((Boolean)state.getValue(POWERED).booleanValue() && !worldIn.isBlockPowered(pos))
            {
                worldIn.setBlockState(pos, state.withProperty(POWERED, Boolean.valueOf(false)).withProperty(FACING, state.getValue(FACING)));
